...
import { checkClientesHealth, CLIENTES_API_URL, registerCliente, registerClienteWithGoogle } from '../services/clientes';
...
const ClienteRegisterPage: React.FC = () => {
  ...
  const canSubmit = useMemo(() => {
    if (!formData.nombreCompleto.trim()) return false;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(getErrorMessage(error, t('No se pudo crear la cuenta.', 'Could not create account.')));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleCredential = useCallback(async (credential?: string) => {
    ...
    try {
      ...
      setError(getErrorMessage(error, t('No se pudo crear la cuenta con Google.', 'Could not create account with Google.')));
    } finally {
      setLoading(false);
    }
        ...
        <Paper elevation={3} sx={{ p: { xs: 3, md: 4 }, width: '100%' }}>
          <Box textAlign="center" mb={2.5}>
            ...
            <Typography variant="h4" component="h1" gutterBottom>
              {t('Crear cuenta (Cliente)', 'Create account (Customer)')}
            </Typography>
            ...
            <Typography variant="body1" color="text.secondary">
              {t('Regístrate con tu correo o continúa con Google', 'Sign up with email or continue with Google')}
            </Typography>
          </Box>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <TextField
              ...
              helperText={
                !!formData.email && !isEmailValid(formData.email)
                  ? t('Correo inválido', 'Invalid email')
                  : ' '
              }
            />
            <TextField
              ...
              InputProps={{
                endAdornment: (
                  <InputAdornment position="end">
                    <IconButton
                      aria-label={
                        showConfirmPassword ? t('Ocultar contraseña', 'Hide password') : t('Mostrar contraseña', 'Show password')
                      }
            ...
            <Button
              ...
              sx={{ mt: 2 }}
            >
              {loading ? <CircularProgress size={24} color="inherit" /> : t('Crear cuenta', 'Create account')}
            </Button>
            ...
            >
              {t('¿Ya tienes cuenta? Inicia sesión', 'Already have an account? Sign in')}
            </Button>
            ...
            <Button
              ...
              onClick={() => navigate('/login')}
            >
              {t('¿Eres trabajador? Inicia sesión', 'Staff? Log in')}
            </Button>
          </form>
        </Paper>
      </Container>
